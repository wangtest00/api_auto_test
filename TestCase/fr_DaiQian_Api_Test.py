...
class FR_DaiQian_Api_Test(unittest.TestCase):
    ...
    def test_get_address_postalCode(self):
        ...
        self.assertEqual(t['errorCode'],0)
        print(t['data'])
        self.assertEqual(t['data'],{'postalCode': '98209', 'area': 'Avalos', 'province': 'Concepción del Oro', 'state': 'Zacatecas', 'stateCode': '11130032'})
        r2=requests.get(host_api+"/api/mexico/get/address/01000",headers=head,verify=False)
        t2=r2.json()
        self.assertEqual(t2['errorCode'],0)
        self.assertEqual(t2['data'],{'postalCode': '01000', 'area': 'San Angel', 'province': 'Álvaro Obregón', 'state': 'Distrito Federal', 'stateCode': None})
    @classmethod
