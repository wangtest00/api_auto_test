...
class FR_DaiHou_Api_Test(unittest.TestCase):
    ...
    def test_loan_latest_08(self):
        ...
        self.assertEqual(str(t['data']['trailPaymentDetail'][0]['loanDays']),"7")
        self.assertEqual(str(t['data']['trailPaymentDetail'][0]['feeList']),"[{'feeName': 'Credit Fee', 'feeValue': '100', 'originalFeeValue': None, 'realRepayAmt': None, 'reduceAmt': None, 'order': '2', 'feeType': 'BEFORE'}, {'feeName': 'fee', 'feeValue': '100', 'originalFeeValue': None, 'realRepayAmt': None, 'reduceAmt': None, 'order': '5', 'feeType': 'BEFORE'}, {'feeName': 'Interest', 'feeValue': '120', 'originalFeeValue': None, 'realRepayAmt': None, 'reduceAmt': None, 'order': '5', 'feeType': 'AFTER'}, {'feeName': 'post fee', 'feeValue': '50', 'originalFeeValue': None, 'realRepayAmt': None, 'reduceAmt': None, 'order': '6', 'feeType': 'BEFORE'}, {'feeName': 'OXXO Fee', 'feeValue': '20', 'originalFeeValue': None, 'realRepayAmt': None, 'reduceAmt': None, 'order': '77', 'feeType': 'AFTER'}]")
        self.assertIsNone(t['data']['repaymentDetail'])
        self.assertIsNone(t['data']['reapplyDate'])
        self.assertIsNone(t['data']['applyButtonDetail'])
    ...
    def test_stp_repayment_02(self):
        '''【FeriaRapida】/api/trade/stp_repayment/annon/event/webhook-还款接口-STP模拟银行回调-有在贷（正常）验证结清-正案例'''
        registNo=cx_registNo_052()
        payment_id=str(random.randint(100000000,999999999))
        if registNo is None:
            registNo=cx_registNo_04()
            print(registNo)
    ...
    def test_oxxo_repayment_02(self):
        '''【FeriaRapida】/api/trade/conekta/annon/event/webhook-还款接口-OXXO模拟银行回调-有在贷(逾期)验证结清(先申请还款后模拟还款回调)-正案例'''
        registNo=cx_registNo_042()
        print(registNo)
        phone=registNo[0]
        custNo=registNo[1]
